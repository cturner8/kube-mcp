...
func TestListIngresses_NoNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	setupMcpEnv(t)

	session := setupMcpServerClient(t, ctx, tools.ListIngressesTool, tools.ListIngressesHandler)
	defer session.Close()

	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name: tools.ListIngressesTool.Name,
	})
	if err != nil {
	...
	if textContent.Text == "" {
		t.Fatalf("expected non-empty ingresses list, got empty string")
	}
...
func TestListIngresses_WithNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
