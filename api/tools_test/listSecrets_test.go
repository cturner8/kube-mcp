...
func TestListSecrets_NoNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	setupMcpEnv(t)

	session := setupMcpServerClient(t, ctx, tools.ListSecretsTool, tools.ListSecretsHandler)
	defer session.Close()

	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name: tools.ListSecretsTool.Name,
	})
	if err != nil {
	...
	if textContent.Text == "" {
		t.Fatalf("expected non-empty secrets list, got empty string")
	}
...
func TestListSecrets_WithNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
