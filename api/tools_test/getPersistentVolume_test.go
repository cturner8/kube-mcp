...
func TestGetPersistentVolume(t *testing.T) {
	...
	session := setupMcpServerClient(t, ctx, tools.GetPersistentVolumeTool, tools.GetPersistentVolumeHandler)
	defer session.Close()

	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name:      tools.GetPersistentVolumeTool.Name,
		Arguments: map[string]any{"name": "test-pv"},
	})
	if err != nil {
	...
	if textContent.Text == "" {
		t.Fatalf("expected non-empty persistent volume, got empty string")
	}
