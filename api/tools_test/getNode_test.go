...
func TestGetNode(t *testing.T) {
	...
	session := setupMcpServerClient(t, ctx, tools.GetNodeTool, tools.GetNodeHandler)
	defer session.Close()

	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name:      tools.GetNodeTool.Name,
		Arguments: map[string]any{"name": "test-node"},
	})
	if err != nil {
	...
	if textContent.Text == "" {
		t.Fatalf("expected non-empty node, got empty string")
	}
