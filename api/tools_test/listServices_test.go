...
func TestListServices_NoNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	setupMcpEnv(t)

	session := setupMcpServerClient(t, ctx, tools.ListServicesTool, tools.ListServicesHandler)
	defer session.Close()

	result, err := session.CallTool(ctx, &mcp.CallToolParams{
		Name: tools.ListServicesTool.Name,
	})
	if err != nil {
	...
	if textContent.Text == "" {
		t.Fatalf("expected non-empty services list, got empty string")
	}
...
func TestListServices_WithNamespace(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
